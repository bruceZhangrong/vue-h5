    ...
    <div class="home-contain-wrapper" ref="containWrapper">
      ...
      <section class="main-content">
        <ul class="main-nav bg-fff">
          <li v-for="(v, k) in mainNav" :key="k">
            <i :class="`fa fa-2x fa-${v.clasIcon}`"></i>
            <p>{{v.title}}</p>
          </li>
        </ul>
      </section>
      <HomeList @add-offset="addOffset" :loadDatas="loadDatas"></HomeList>
    </div>
  </div>
</template>
  ...
  methods: {
    changeOpacity (event) {
      let opacity = parseFloat(
        event.target.scrollTop / (this.scrollHeight - 40)
      )
      this.$store.dispatch('changeHeader', {
        isBorder: false,
        isBgColor: true,
      ...
      let distance =
        this.$refs.containWrapper.offsetHeight -
        (this.viewHeight + event.target.scrollTop)
