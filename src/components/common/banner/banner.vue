<template>
  ...
  >
    <ul ref="ulWrap" class="banner-wrap">
      <li v-for="(v, k) in imgs" :key="k" ref="v_li" :data-id="'li'+k">
        <img :src="v" alt="banner" />
      </li>
    </ul>
    <div class="v-point" v-if="pointShow">
      <span v-for="k in imgs" :key="k" ref="point"></span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    init () {
      ...
      this.$refs.v_li[this.liLength - 1].style.transform = `translateX(${
        this.translate_x[this.liLength - 1]
      }px)`
      this.setTimer()
    },
    touchStart (event) {
    ...
    slideLeftOrRight (slideDirection) {
      // 左滑动 -1; 右滑动 1;
      if (this.pointShow) {
      ...
      for (let v = 0; v < this.$refs.v_li.length; v++) {
        ...
        this.$refs.v_li[v].style.transitionDuration = '300ms'
        if (
          this.translate_x[v] === slideDirection * ((this.liLength - 1) * this.winWidth)
        ) {
          this.translate_x[v] = -(slideDirection * this.winWidth)
          this.$refs.v_li[v].style.transitionDuration = '0ms'
        }
